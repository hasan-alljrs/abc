    ...
    match /{document=**}  {
      function validateAuth() {
  			return request.auth != null 
               ...
               && request.auth.token.email.matches("[a-zA-Z0-9_\\.\\~\\%\\#\\&\\!\\=\\?\\$\\'\\*\\+\\/\\^\\`\\{\\}\\|\\-\\]*@[a-z]*.[a-z]*")
               && request.auth.token.oid != null
               && request.auth.token.oid.size() != 0;
    	}
      function checkId(targetResource) {
      	return targetResource == null
        			 		? true
                  : targetResource.data.id is string
        							? targetResource.data.id == request.auth.token.oid
               				: targetResource.data.id.oid == request.auth.token.oid;
      }
      // allow read, delete: if validateAuth() && checkId(resource);
      // allow create, update: if validateAuth() && checkId(request.resource); 
...
- `GMAIL_PASSWORD` を直接使用せず、Gmail などのメール配信サービスの利用を検討
